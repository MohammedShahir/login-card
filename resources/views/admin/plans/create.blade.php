...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-3xl font-bold">إضافة باقة جديدة</h2>
    </div>
    ...
    <div class="glass-panel p-6 rounded-3xl border-brand-purple/20">
        <form action="{{ route('admin.plans.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="مثال: بطاقة 5 ريال" class="glass-input w-full px-4 py-2.5 rounded-xl">
            </div>
            <div>
                <label class="block text-sm font-medium mb-1.5 text-white/80">السعر</label>
                <input type="number" step="0.01" name="price" value="{{ old('price') }}" required class="glass-input w-full px-4 py-2.5 rounded-xl">
            </div>
            
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm font-medium mb-1.5 text-white/80">السعة (جيجابايت)</label>
                    <input type="number" name="data_limit" value="{{ old('data_limit') }}" placeholder="اتركه فارغاً للاستخدام المفتوح" class="glass-input w-full px-4 py-2.5 rounded-xl">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1.5 text-white/80">الوقت (بالدقائق)</label>
                    <input type="number" name="time_limit" value="{{ old('time_limit') }}" placeholder="اتركه فارغاً للوقت المفتوح" class="glass-input w-full px-4 py-2.5 rounded-xl">
                </div>
            </div>
            
            <div>
                <label class="block text-sm font-medium mb-1.5 text-white/80">تحديد السرعة (MikroTik Rate Limit)</label>
                <input type="text" name="speed_limit" value="{{ old('speed_limit') }}" dir="ltr" placeholder="مثال: 5M/1M" class="glass-input w-full px-4 py-2.5 rounded-xl font-mono">
            </div>
            
            <button type="submit" class="glass-button w-full py-3.5 rounded-xl bg-brand-purple/20 text-brand-purple font-bold hover:bg-brand-purple/30 mt-6 shadow-lg shadow-brand-purple/10">حفظ الباقة</button>
        </form>
    </div>
</div>
@endsection
